...
namespace lib
{
    namespace
    {
        constexpr std::array<std::uint32_t, 8> blake2s_iv { 0x6A09E667, 0xBB67AE85, 0x3C6EF372,
        ...
        constexpr void g_mix(
            std::array<std::uint32_t, 16> &v, std::size_t a, std::size_t b, std::size_t c,
            std::size_t d, std::uint32_t x, std::uint32_t y
        )
        {
            v[a] += v[b] + x;
    ...
    void blake2s(
        std::span<std::byte> out, std::span<const std::byte> input, std::span<const std::byte> key
    )
    {
        blake2s_state state;
        if (key.empty())
