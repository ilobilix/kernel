...
namespace lib::detail
{
    ...
    class locked
    {
        ...
        public:
        locked(Type *_ptr, Lock &_lock) : _ptr { _ptr }, _lock { _lock }, _locked { false } { lock(); }
    ...
    class storage
    {
        ...
        static consteval bool safety()
        {
            ...
            static_assert(
                __builtin_offsetof(buffer, _lock) == __builtin_offsetof(Other::buffer, _lock)
            );
            static_assert(
                __builtin_offsetof(buffer, _buffer) == __builtin_offsetof(Other::buffer, _buffer)
            );
            return true;
        }
        ...
        template<typename Type1>
            requires(safety<Type1>())
        constexpr storage(const storage<Type1, Lock> &rhs)
            : _data { rhs._data }, _ptr { static_cast<Type *>(rhs._ptr) }
        { }

        template<typename Type1>
            requires(safety<Type1>())
        constexpr storage(storage<Type1, Lock> &&rhs)
            : _data { std::move(rhs._data) }, _ptr { static_cast<Type *>(rhs._ptr) }
        { }
...
export namespace lib
{
    ...
    class locked_ptr
    {
        ...
        constexpr locked_ptr(const locked_ptr &rhs) : _storage { rhs._storage } { }

        constexpr locked_ptr(locked_ptr &&rhs) : _storage { std::move(rhs._storage) } { }

        // type checks in storage::storage()
        template<typename Type1>
        constexpr locked_ptr(const locked_ptr<Type1, Lock> &rhs) : _storage { rhs._storage }
        { }

        template<typename Type1>
        constexpr locked_ptr(locked_ptr<Type1, Lock> &&rhs) : _storage { std::move(rhs._storage) }
        { }
        ...
        template<typename Self>
            requires detail::is_rwlock<Lock>
    ...
    {
        return locked_ptr<Type, Lock> { detail::make_locked_tag, std::forward<Args>(args)... };
    }
    ...
    class locker
    {
        ...
        ~locker() { std::destroy_at(valueptr()); }
        ...
        {
            return detail::locked<Type, Lock, true> {
                std::forward<Self>(self).valueptr(), std::forward<Self>(self)._lock
            };
        }

        bool is_locked() const { return _lock.is_locked(); }
