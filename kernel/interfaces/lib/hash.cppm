...
export namespace lib::hash
{
    ...
    uint128_t murmur3_128(
        const void *key, std::uint64_t len, std::uint64_t seed, std::uint64_t *out = nullptr
    )
    {
        auto rotl64 = [](std::uint64_t x, std::int8_t r) { return (x << r) | (x >> (64 - r)); };
        ...
        auto tail =
            static_cast<const std::uint8_t *>(static_cast<const void *>(data + nblocks * 16));
